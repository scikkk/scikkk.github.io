...
import numpy as np
import mmcv
import imageiodef convert_rgb_videos(video_file, save_file):
...
def convert_updown_leftright(video_file, save_file):
    ...
    imageio.mimsave(save_file, new_frames, fps=30)def combine_videos(video_file_1, video_file_2, save_file):
    ...
    for f1, f2 in zip(frames_1, frames_2):
        f = np.vstack([f1, f2])
        frames.append(f)
    imageio.mimsave(save_file, frames, fps=30)dir = 'static/videos'
