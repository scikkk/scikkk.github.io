(() => {
  ...
  (() => {
    ...
    if (window.addEventListener("hashchange", k), $("#navbar-main li.nav-item a.nav-link, .js-scroll").on("click", function (b2) {
      ...
      b.isotope({
        itemSelector: ".isotope-item", percentPosition: true, masonry: { columnWidth: ".grid-sizer" }, filter: function () {
    ...
    $(document).ready(function () {
      ...
      $(".pub-filters-select") && M(), $(".js-cite-modal").click(function (c3) {
        ...
        b3.preventDefault();
        let c3 = document.querySelector("#modal .modal-body");
        // wangke
        if (c3) {
          // 获取要复制的文本
          const textToCopy = c3.innerText || c3.textContent;
          // 使用 Clipboard API 进行复制
          navigator.clipboard.writeText(textToCopy).then(() => {
            console.log("Citation copied successfully!");
          }).catch(err => {
            console.log("Error: citation copy failed.", err);
          });
        } else {
